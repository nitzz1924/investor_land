...
@section('title',$projectdetails->projectname)
@section('content')
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="page-header-box">
                    <h1 class="text-anime">{{$projectdetails->projectname}}</h1>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="post-content">
                    <div class="post-featured-image wow fadeInUp" data-wow-delay="0.25s">
                        <figure class="image-anime">
                            <img height="700px" src="{{asset('assets/images/Projects/'.$projectdetails->projectthumbnail)}}" alt="">
                        </figure>
                    </div>
                    ...
                    <div class="row align-items-center">
                        <div class="col-lg-8">
                            <div class=" wow fadeInUp" data-wow-delay="0.25s">
                                Posted on :
                                <a class="fw-bold text-black">{{$projectdetails->created_at->format('d/M/Y')}}</a>
                            </div>
                        </div>
                    ...
                    <div class="post-entry wow fadeInUp mt-4" data-wow-delay="0.5s">
                        @php
                        $cleanedContent = preg_replace('/contenteditable="[^"]*"/', '', $projectdetails->projectdescription);
                        @endphp
                        <p> {!! $cleanedContent !!}</p>
                    </div>
            ...
            <div class="col-lg-4">
                <div class="post-sidebar">
                    <div class="category-box-widget wow fadeInUp" data-wow-delay="0.5s">
                        ...
                        <ul>
                            @php
                            $categories = json_decode($projectdetails->categories ?? '[]', true);
                            @endphp
                    ...
                    <div class="recent-posts-widget wow fadeInUp" data-wow-delay="0.75s">
                        ...
                        <div class="recent-posts-lists">
                            @foreach ($receentproject->take(3) as $latest)
                            <div class="recent-post-item">
                                <div class="post-image">
                                    <img src="{{asset('assets/images/Projects/'.$latest->projectthumbnail)}}" alt="">
                                </div>

                                <div class="post-info">
                                    <p class="meta"><a href="{{ route('website.blogdetails',['id' => $latest->id])}}">{{ implode(', ', json_decode($latest->categories)) }}</a></p>
                                    <h4><a href="{{ route('website.projectdetails',['id' => $latest->id])}}">{{Str::limit($latest->projectname,15)}}</a></h4>
                                </div>
                            </div>
                            @endforeach
                        </div>
